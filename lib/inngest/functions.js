    ...
    async ({ event, step }) => {
        ...
        try {
            ...
            const sandboxId = await step.run("create-sandbox", async () => {
                const sandbox = await Sandbox.create("qzv1h78rnvftdizmkfay", {
            ...
            // Step 4: Store the result so the client can poll for it
            setBuild(eventId, {
                sandboxId,
                url: sandboxURL,
                status: "ready",
            ...
            // Store the error so the client gets feedback instead of polling forever
            setBuild(eventId, {
                status: "error",
